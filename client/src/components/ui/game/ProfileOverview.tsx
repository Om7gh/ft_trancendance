...
export default function ProfileOverview({ data }: { data: ProfileDataType[] }) {
  if (!data.length)
    return (
      <div className="p-6 text-center bg-slate-800/50  border border-slate-700">
        <p className="text-slate-400">Select a player to view profile</p>
      </div>
    );

  return (
    <div className="bg-slate-900/80 backdrop-blur-sm border border-slate-700  overflow-hidden shadow-lg transition-all duration-300 hover:shadow-violet-500/10">
      <div className="bg-gradient-to-r from-violet-900/30 to-slate-900 p-6">
        <div className="flex flex-col items-center">
          <div className="relative mb-4">
            ...
            />
            <div className="absolute inset-0 bg-violet-500 rounded-full blur-md opacity-20 animate-pulse"></div>
          </div>

          <div className="text-center">
            <h2 className="text-xl font-bold text-white">{data[0].username}</h2>
            <span className="inline-block mt-1 px-3 py-1 text-xs bg-slate-800 text-violet-400 rounded-full">
              Pro Player
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 p-4 bg-slate-800/30">
        <div className="bg-slate-800/60  p-3 text-center transition-transform hover:scale-105">
          <p className="text-xs uppercase tracking-wider text-violet-400">
            Wins
          </p>
          <p className="text-2xl font-bold text-white">56%</p>
          <div className="h-1 mt-2 bg-violet-500/30 rounded-full">
            <div
      ...
      <div className="p-4 flex gap-3 bg-slate-900/50">
        <Button
          type="signIn"
          className="flex-1 bg-gradient-to-r from-violet-600 to-violet-500 hover:from-violet-500 hover:to-violet-400 text-white shadow-lg shadow-violet-500/20"
        >
          Challenge
        </Button>
        <Button
