  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 overflow-scroll max-h-screen">
      <div className="mb-10 flex justify-between items-center">
        <div>
          <h1 className="text-4xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-violet-400 to-neon leading-tight">
            Player Profile
          </h1>
          <div className="h-1.5 bg-gradient-to-r from-violet-500 via-violet-400 to-neon w-full max-w-xs rounded-full mt-2" />
        </div>
        <Link
          to="/dashboard/settings"
          className="p-2 rounded-full hover:bg-slate-800/50 transition-all duration-200 group"
        >
          <IoSettings className="text-2xl text-slate-400 group-hover:text-neon transition-colors duration-200" />
        </Link>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-slate-900/50 backdrop-blur-sm  p-6 lg:p-8 shadow-xl border border-slate-800 mb-10">
        <div className="flex flex-col lg:flex-row items-center gap-6 lg:border-r lg:border-slate-700 lg:pr-8">
          <div className="relative group">
            <div className="w-40 h-40 rounded-full overflow-hidden border-4 border-neon/20 shadow-xl transition-all duration-300 group-hover:border-neon/40">
              <img
                src={Avatar}
                alt="avatar"
            ...
            <div className="absolute inset-0 rounded-full border-4 border-transparent group-hover:border-violet-400/20 pointer-events-none transition-all duration-300" />
          </div>
          ...
          <div className="text-center lg:text-left space-y-2">
            ...
            <div className="inline-flex items-center px-3 py-1 rounded-full bg-gradient-to-r from-neon/10 to-neon/20 border border-neon/30">
              <span className="h-2 w-2 rounded-full bg-neon mr-2 animate-pulse" />
              <span className="text-neon font-medium">Beginner Player</span>
            </div>
        ...
        <div className="space-y-6">
          <h3 className="text-xl font-semibold text-slate-200 pb-2 border-b border-slate-700 flex items-center">
            <span className="bg-gradient-to-r from-violet-400 to-neon bg-clip-text text-transparent">
              Player Information
            </span>
          </h3>
      ...
      <div className="bg-slate-900/80 backdrop-blur-sm  p-6 lg:p-8 shadow-xl border border-slate-800">
...
function StatCard({
  ...
  return (
    <div className="bg-slate-900/80 hover:bg-slate-900 backdrop-blur-sm  p-6 shadow-lg border border-slate-800 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
      <div
        className={`w-12 h-12 ${accent}  flex items-center justify-center mb-4`}
      >
        {icon}
      </div>
      <h3 className="text-xl font-semibold text-slate-200 mb-1">{title}</h3>
      <div className="text-4xl font-bold text-neon mb-2">{value}</div>
    ...
    <div className="flex items-start py-3 border-b border-slate-800 last:border-0 group">
      <div
        className={`w-3 h-3 rounded-full mt-1.5 mr-3 flex-shrink-0 ${
          positive ? 'bg-violet-500' : 'bg-rose-500'
        }`}
      />
          ...
          <p className="text-slate-400 text-sm">
            ...
            <span
              className={`font-medium ${
                positive ? 'text-violet-400' : 'text-rose-400'
              }`}
            >
              {score}
            </span>
