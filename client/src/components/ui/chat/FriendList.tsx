...
export default function FriendList({ data }: UserData[]) {
  ...
  return (
    <div
      ...
      } overflow-scroll bg-slate-900/40 p-3  shadow-sm shadow-slate-700/50`}
    >
      {!isMobile && (
        <div className="flex items-center gap-3 border-b border-b-slate-500 p-5">
          <img
            src={Avatar}
            alt="root"
            className=" w-25 border border-violet-500 p-1 h-auto rounded-full  object-cover "
          />
      ...
      >
        {data &&
          data.map((data) => (
            <li
              key={data.id}
              className={`flex items-center justify-start gap-5 my-5 bg-slate-950/10 cursor-pointer hover:bg-slate-950/20 transition-all duration-150  p-2`}
            >
              <img
                src={Avatar}
                alt="avatar"
                className={`${
                  isMobile ? 'w-15' : 'w-20'
                } border border-violet-500 p-1 h-auto rounded-full object-cover ml-6`}
              />
              <div>
                {!isMobile && (
                  <>
