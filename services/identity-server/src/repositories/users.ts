...
export class UserRepository {
  ...
  insert(user: Omit<User, 'id'>): User {
    const result = this.db
      .prepare(
        `INSERT INTO users (username, email, first_name, last_name, password, avatar, bio, last_login, email_verified, provider, token_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(
        user.username,
  ...
  findByUID(uid: string): User | null {
    return this.db.prepare('SELECT * FROM users WHERE uid = ?').get(uid) as User | null
  }
  ...
  update(id: number, data: Partial<Omit<User, 'id' | 'created_at'>>): void {
    ...
    const result = this.db
      .prepare(`UPDATE users SET ${updates.join(', ')}, updated_at = ? WHERE id = ?`)
      .run(...values)
    if (result.changes === 0) {
