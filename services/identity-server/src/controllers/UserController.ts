    ...
    try {
      ...
      for await (const part of request.parts()) {
        if (part.type === "file" && part.fieldname === "avatar") {
          ...
          payload.avatar = await saveUploadedAvatar(user.uid, user.username, {
            ...part,
        ...
        if (
          ...
          return reply.badRequest(value + UserController.ERR_INVALID_FIELD);
        }
  ...
  static async user(
    ...
    try {
      ...
      const fullUser = {
        ...
        pong: await UserController.getStatistics("http://pong:9001", user.uid),
