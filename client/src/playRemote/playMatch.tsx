...
function messageHandler(event: MessageEvent, setMatchState: ((value: string) => void), ) {
...
function createConnection(connection: {ws: WebSocket | null}, setMatchState: ((value: string) => void), url: string) {
    ...
    connection.ws.onerror   = (     ) => setMatchState("Error happens with connection!!");
...
export function PlayMatch({ match }: PlayMatchPropsType) {
    ...
    if ((matchState === "going") || (matchState === "done")) {
        return <Match connection={connection.current.ws!} matchState={matchState} setMatchState={setMatchState} />;
    }
