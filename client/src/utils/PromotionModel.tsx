...
const PromotionModal = ({
  ...
  return (
    <div className="fixed top-0 left-0 right-0 bottom-0 bg-slate-900/50 backdrop-blur-sm flex justify-center items-center z-50 w-full m-auto">
      <div className="flex justify-evenly bg-neon-400/80 backdrop-blur-md  w-[40vmax] py-8">
        {promotionPieces.map((pieceType) => {
          let pieceImage;
          switch (pieceType) {
            case 'QUEEN':
              pieceImage = teamType === 0 ? Piece('wQ') : Piece('bQ');
              break;
            case 'ROCK':
              pieceImage = teamType === 0 ? Piece('wR') : Piece('bR');
              break;
            case 'BISHOP':
              pieceImage = teamType === 0 ? Piece('wB') : Piece('bB');
              break;
            case 'KNIGHT':
              pieceImage = teamType === 0 ? Piece('wN') : Piece('bN');
              break;
            default:
